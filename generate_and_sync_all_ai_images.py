...
def generate_all_images():
    print(f"=== BẮT ĐẦU TẠO 8 ẢNH BẰNG FLUX / POLLINATIONS AI ===")
    for item in PROMPTS_DATA:
        ...
        url = f"https://image.pollinations.ai/prompt/{encoded_prompt}?width={w}&height={h}&nologo=true&model=flux"
        
        dest_folder_path = os.path.join(OUTPUT_DIR, filename)
        dest_root_path = os.path.join(BASE_DIR, filename)
        
        print(f"\n[#{p_id}/8] Đang tạo: {filename} ({w}x{h})...")
        success = False
                ...
                with urllib.request.urlopen(req, timeout=90) as response:
                    ...
                    if len(img_data) > 1000:
                        ...
                        with open(dest_root_path, "wb") as f:
                            f.write(img_data)
                        print(f" -> Đã lưu thành công: {filename} ({len(img_data)} bytes)")
                        success = True
                        break
            except Exception as e:
                print(f" -> Thử lại lần {attempt+1} do lỗi: {e}")
                time.sleep(2)
